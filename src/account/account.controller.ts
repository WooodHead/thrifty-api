...
export class AccountController {
  ...
  @ApiOkResponse({
    description:
      "SUCCESS: All Accounts for the specified user on the server returned",
  })
  @ApiUnauthorizedResponse({
    description:
  ...
  async checkAccountBalance(
    @Query() accountNumberDto: AccountNumberDto,
    @UserDecorator("id") id: string
  ) {
    const { accountNumber } = accountNumberDto;

    const responseData = await this.accountService.checkAccountBalance(
      +accountNumber,
      id
    );
    ...
    return new SuccessResponse(
      200,
      "Account Retrieved By Account Number",
      responseData
    );
  }
  ...
  @ApiOperation({
    description:
      "Searches for an account by account number belonging to the authenticated user",
  })
  @ApiOkResponse({
    description:
      "SUCCESS: Account with the specified account number on the server returned",
  })
  @ApiBadRequestResponse({
  ...
  @ApiNotFoundResponse({
    description:
      "Account with the specified accountNumber does not exist on the server",
  })
  @ApiInternalServerErrorResponse({
    description: "An Internal Error Occurred while processing the request",
  })
  async findByAccountNumber(
    ...
    const responseData = await this.accountService.findByAccountNumber(
      +accountNumber,
      id
    );
  ...
  @ApiOperation({
    description:
      "Searches for an account by account name, belonging to the authenticated user",
  })
  @ApiOkResponse({
    description:
      "SUCCESS: Account with the specified account name on the server returned",
  })
  @ApiBadRequestResponse({
    description: "Request Parameter is empty or contains unacceptable values",
  })
  ...
  async findByAccountName(
    @Query() accountNameDto: AccountNameDto,
    @UserDecorator("id") id: string
  ) {
    const { accountName } = accountNameDto;

    const responseData = await this.accountService.findByAccountName(
      accountName,
      id
    );
    ...
    return new SuccessResponse(
      200,
      "Account Retrieved By Account Name",
      responseData
    );
  }
  ...
  @ApiOperation({
    description:
      "Opens a new Account, Account number is auto-generated internally",
  })
  @ApiCreatedResponse({
    description:
      "SUCCESS: New Account opened with the details in the request body",
  })
  @ApiBadRequestResponse({
    description:
  ...
  async depositFunds(
    @Body() transactionInfo: DepositOrWithdrawMoneyDto,
    @UserDecorator() user: User
  ) {
    const responseData = await this.accountService.depositFunds(
      transactionInfo,
      user
    );
  ...
  @ApiOkResponse({
    description:
      "SUCCESS: Account debited and Funds withdrawn from the specified account",
  })
  @ApiBadRequestResponse({
    description: "Request Body is empty or contains unacceptable values",
  })
  ...
  @ApiForbiddenResponse({
    description:
      "Invalid Account Name/Number combination OR Insufficient balance to carry out the transaction",
  })
  @ApiInternalServerErrorResponse({
    description: "An Internal Error Occurred while processing the request",
  })
  async withdrawFunds(
    ...
    const responseData = await this.accountService.withdrawFunds(
      transactionInfo,
      user
    );
  ...
  async internalTransfer(
    @Body() transferInternalDto: TransferFundsToInternalDto,
    @UserDecorator() user: User
  ) {
    const responseData = await this.accountService.internalFundsTransfer(
      transferInternalDto,
      user
    );
  ...
  async externalTransfer(
    @Body() transferExternalDto: TransferFundsToExternalDto,
    @UserDecorator() user: User
  ) {
    const responseData = await this.accountService.externalFundsTransfer(
      transferExternalDto,
      user
    );
  ...
  async payBills(@Body() payBillDto: PayBillsDto, @UserDecorator() user: User) {
    const responseData = await this.accountService.billPayment(
      payBillDto,
      user
    );
  ...
  update(
    ...
    const responseData = this.accountService.update(
      accountNumber,
      updateAccountDto,
      id
    );
