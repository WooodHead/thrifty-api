...
export class updateUserEntityRefreshTokenLogic1652262018638
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."user_roles_enum" AS ENUM('guest', 'user', 'admin', 'super_admin')`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE "user" DROP COLUMN "refreshToken"`);
    await queryRunner.query(
      `ALTER TABLE "user" ADD "refreshToken" jsonb DEFAULT '{}'`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."user_roles_enum_old" AS ENUM('user', 'admin', 'super-admin')`
    );
    await queryRunner.query(
      `ALTER TABLE "user" ALTER COLUMN "roles" DROP DEFAULT`
    );
