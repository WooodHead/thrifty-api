...
export class AddedTransactionAndAccountEntity1652351121053
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."transaction_transactiontype_enum" AS ENUM('INSTANT TRANSFER', 'BILL PAYMENT')`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."transaction_transactionmode_enum" AS ENUM('DEBIT', 'CREDIT')`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."transaction_transactionstatus_enum" AS ENUM('PENDING', 'APPROVED', 'REJECTED', 'CANCELLED', 'FAILED', 'SUCCESSFUL', 'REVERSED', 'REVERSAL-REQUESTED', 'REVERSAL-APPROVED', 'REVERSAL-REJECTED', 'REVERSAL-CANCELLED', 'REVERSAL-FAILED', 'REVERSAL-SUCCESSFUL')`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."account_accounttype_enum" AS ENUM('CORPORATE_CURRENT', 'CORPORATE_DOMICILIARY', 'CORPORATE_SAVINGS', 'INDIVIDUAL_CURRENT', 'INDIVIDUAL_DOMICILIARY', 'INDIVIDUAL_SAVINGS', 'GROUP_CURRENT', 'GROUP_SAVINGS', 'JOINT_CURRENT', 'JOINT_SAVINGS')`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."account_accountcurrency_enum" AS ENUM('NGN', 'USD', 'EUR', 'GBP', 'JPY', 'AUD', 'CAD', 'CHF', 'CNY')`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."account_accountstatus_enum" AS ENUM('ACTIVE', 'DORMANT', 'SUSPENDED')`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."user_roles_enum" AS ENUM('ADMIN', 'GUEST', 'SUPER_ADMIN', 'SYSTEM_ADMIN', 'USER')`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TYPE "public"."account_accountcurrency_enum"`
    );
    ...
    await queryRunner.query(
      `DROP TYPE "public"."transaction_transactionmode_enum"`
    );
    await queryRunner.query(
