...
export class SavingsGroupService {
  constructor(
    ...
    @InjectRepository(UserToSavingsGroup)
    private readonly userToSavingsGroupRepository: Repository<UserToSavingsGroup>
  ) {}
  ...
  async findByName(name: string, adminUser: User) {
    try {
      ...
      throw new NotFoundException(
        `Savings Group with groupName: ${name} not found on this server`
      );
    } catch (error) {
      console.error(error);
  ...
  async create(createSavingsGroupDto: CreateSavingsGroupDto, user: User) {
    try {
      ...
      const newSavingsGroup = this.savingsGroupRepository.create(
        createSavingsGroupDto
      );
  ...
  async addSavingsGroupMember(
    addMemberDto: UpdateGroupMemberDto,
    adminUser: User
  ) {
    try {
      ...
      if (!userToAdd) {
        throw new NotFoundException(
          `User with ID: ${userId} not found on this server`
        );
      }
      ...
      if (!savingsGroup) {
        throw new NotFoundException(
          `SavingsGroup with ID: ${savingsGroupId} not found on this server`
        );
      }
      ...
      if (savingsGroup.groupAdmin.id === adminUser.id) {
        ...
        const userAlreadyAdded =
          await this.userToSavingsGroupRepository.findOne({
            where: { ...addMemberDto },
          });

        if (userAlreadyAdded) {
          throw new ConflictException(
            `User with ID: ${userId} already belongs to savingsGroup with ID: ${savingsGroupId}`
          );
        }

        // add user to savingsGroup
        const addUserToGroup =
          this.userToSavingsGroupRepository.create(addMemberDto);
  ...
  async removeSavingsGroupMember(
    removeMemberDto: UpdateGroupMemberDto,
    adminUser: User
  ) {
    try {
      ...
      if (savingsGroup.groupAdmin.id === adminUser.id) {
        ...
        const userIsMember = groupMembers.some(
          (member) => member.userId === user.id
        );

        if (!userIsMember) {
          throw new NotFoundException(
            `User with ID: ${userId} is not a member of savingsGroup with ID: ${savingsGroupId}`
          );
        }
  ...
  async contriubeFundsToGroup(
    user: User,
    contibuteFundsDto: ContributeFundsDto
  ) {
    try {
      ...
      saveToGroup.contributedFunds =
        +saveToGroup.contributedFunds + amountToSave;
  ...
  async update(
    id: string,
    updateSavingsGroupDto: UpdateSavingsGroupDto,
    adminUser: User
  ) {
    try {
